...
const SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_Z9qJCJ03PI1RThpbvoS77Q_VtVgc527';
...
async function uploadFile(file, storagePath) {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorText || 'The file upload was rejected by Supabase.');
  }
}
...
async function saveOrder(order) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(errorText || 'Supabase rejected the order.');
  }
}
...
orderForm?.addEventListener('submit', async (event) => {
  ...
  if (selectedFile && selectedFile.size > MAX_FILE_SIZE) {
    alert('The selected file is larger than 50 MB. Please upload a smaller file or ZIP it first.');
    return;
  }
  ...
  try {
    if (selectedFile) {
      ...
      const cleanedName = safeFileName(selectedFile.name) || 'uploaded-file';
      storagePath = `${orderId}/${Date.now()}-${cleanedName}`;
    ...
    alert(
      'We could not submit your request. Please try again. If the problem continues, contact Beyond directly.'
    );

    if (submitButton) {
      submitButton.disabled = false;
      submitButton.innerHTML = originalButtonText || 'Submit print request →';
    }
  }
});

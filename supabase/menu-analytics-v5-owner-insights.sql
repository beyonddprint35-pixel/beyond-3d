-- Rich browsing insights for restaurant and bar owners.
-- Extends the rolling 30-day summary with a weekday/hour heatmap,
-- 7-day comparison data, and enough item visibility rows to analyse menu depth.
...
as $$
  ...
  heatmap_rows as (
    select extract(isodow from (e.occurred_at at time zone 'UTC'))::int as weekday,
      extract(hour from (e.occurred_at at time zone 'UTC'))::int as hour,
      count(*)::bigint as views,
      count(distinct e.session_id)::bigint as unique_sessions
    from filtered e
    where e.event_type = 'menu_view'
  ...
  period_stats as (
    select
      count(*) filter (where event_type = 'menu_view' and occurred_at >= now() - interval '7 days')::bigint as current_views,
      count(distinct session_id) filter (where event_type = 'menu_view' and occurred_at >= now() - interval '7 days')::bigint as current_sessions,
      count(*) filter (where event_type = 'category_view' and occurred_at >= now() - interval '7 days')::bigint as current_category_views,
      count(*) filter (where event_type = 'item_impression' and occurred_at >= now() - interval '7 days')::bigint as current_item_impressions,
      count(*) filter (where event_type = 'menu_view' and occurred_at >= now() - interval '14 days' and occurred_at < now() - interval '7 days')::bigint as previous_views,
      count(distinct session_id) filter (where event_type = 'menu_view' and occurred_at >= now() - interval '14 days' and occurred_at < now() - interval '7 days')::bigint as previous_sessions,
      count(*) filter (where event_type = 'category_view' and occurred_at >= now() - interval '14 days' and occurred_at < now() - interval '7 days')::bigint as previous_category_views,
      count(*) filter (where event_type = 'item_impression' and occurred_at >= now() - interval '14 days' and occurred_at < now() - interval '7 days')::bigint as previous_item_impressions
    from filtered
  )
  select jsonb_build_object(
    ...
    'category_performance', coalesce((
      ...
      from (select * from category_rows order by unique_sessions desc, views desc, entity_id limit 50) r
    ), '[]'::jsonb),
    'item_visibility', coalesce((
      ...
      from (select * from item_rows order by unique_sessions desc, impressions desc, entity_id limit 200) r
    ), '[]'::jsonb),
    ...
    'traffic_by_weekday', coalesce((
      select jsonb_agg(jsonb_build_object('weekday', g.weekday, 'views', coalesce(w.views, 0), 'unique_sessions', coalesce(w.unique_sessions, 0)) order by g.weekday)
      from generate_series(1, 7) g(weekday)
      left join weekday_rows w using (weekday)
    ), '[]'::jsonb),
    ...
    'traffic_heatmap_utc', coalesce((
      select jsonb_agg(jsonb_build_object('weekday', d.weekday, 'hour', h.hour, 'views', coalesce(x.views, 0), 'unique_sessions', coalesce(x.unique_sessions, 0)) order by d.weekday, h.hour)
      from generate_series(1, 7) d(weekday)
      cross join generate_series(0, 23) h(hour)
      left join heatmap_rows x on x.weekday = d.weekday and x.hour = h.hour
    ), '[]'::jsonb),
    'period_comparison', (
      select jsonb_build_object(
        'current_7d', jsonb_build_object('menu_views', current_views, 'unique_sessions', current_sessions, 'category_views', current_category_views, 'item_impressions', current_item_impressions),
        'previous_7d', jsonb_build_object('menu_views', previous_views, 'unique_sessions', previous_sessions, 'category_views', previous_category_views, 'item_impressions', previous_item_impressions)
      )
      from period_stats
    ),

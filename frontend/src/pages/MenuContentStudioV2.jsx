...
const FRIENDLY_COPY = {
  en: {
    yourMenu: "Your menu",
    editMenuHint: "Choose something to edit",
    search: "Search menu",
    ...
    dragHint: "Drag to reorder. Drop an item on another category to move it.",
    addTo: (name) => `Add item to ${name || "category"}`,
  },
  he: {
    yourMenu: "התפריט שלך",
    editMenuHint: "בחרו מה תרצו לערוך",
    search: "חיפוש בתפריט",
    noResults: "לא נמצאו פריטים",
    translations: "תרגומים",
    editTranslations: "עריכת תרגומים",
    hideTranslations: "הסתרת תרגומים",
    moreOptions: "אפשרויות נוספות",
    hideOptions: "הסתרת אפשרויות",
    primaryLanguage: "שפה ראשית",
    dragHint: "גררו כדי לשנות סדר. גררו פריט לקטגוריה אחרת כדי להעביר אותו.",
    addTo: (name) => `הוספת פריט ל${name || "קטגוריה"}`,
  },
  ar: {
    yourMenu: "قائمتك",
    editMenuHint: "اختر ما تريد تعديله",
    ...
    dragHint: "اسحب لتغيير الترتيب. اسحب العنصر إلى فئة أخرى لنقله.",
    addTo: (name) => `إضافة عنصر إلى ${name || "الفئة"}`,
  },
};

const DELETE_CONFIRM_COPY = {
  en: {
    itemTitle: "Delete item?", categoryTitle: "Delete category?", subcategoryTitle: "Delete subcategory?",
  ...
  he: {
    itemTitle: "למחוק את הפריט?", categoryTitle: "למחוק את הקטגוריה?", subcategoryTitle: "למחוק את תת-הקטגוריה?",
    ...
    cancel: "ביטול", confirm: "מחיקה",
  },
  ar: {
    itemTitle: "حذف الصنف؟", categoryTitle: "حذف الفئة؟", subcategoryTitle: "حذف الفئة الفرعية؟",
...
function searchableText(value) {
  if (value && typeof value === "object") return Object.values(value).join(" ").toLocaleLowerCase();
...
function optionLabel(option, language = "en") {
  return String(option?.[`label_${language}`] || option?.label || option?.label_en || option?.label_he || option?.label_ar || "");
}
...
function priceSummary(item, currencySymbol = "₪", language = "en") {
  ...
  if (options.length) return options.map((option) => `${option.label ? `${option.label} ` : ""}${currencySymbol}${option.price}`).join(" · ");
...
function hasLocalizedContent(entry, language, fields) {
  return fields.some((field) => localizedFieldValue(entry?.[field], language).trim());
}
...
export default function MenuContentStudioV2() {
  ...
  const editingLanguages = useMemo(() => {
    const enabled = (Array.isArray(menu.languages) ? menu.languages : []).filter((language) => MENU_LANGUAGE_META[language]);
  ...
  const searchMatches = useMemo(() => {
    ...
    (menu.groups || []).forEach((group) => {
      if (`${searchableText(group.name)} ${searchableText(group.note)}`.includes(searchNeedle)) groupIds.add(group.id);
    });
    (menu.items || []).forEach((item) => {
      if (`${searchableText(item.name)} ${searchableText(item.description)}`.includes(searchNeedle)) {
        itemIds.add(item.id);
        groupIds.add(item.group_id);
        groupIds.add(rootGroupId(menu.groups, item.group_id));
      }
    });
    return { groupIds, itemIds };
  }, [menu.groups, menu.items, searchNeedle]);
  ...
  useEffect(() => {
    ...
    if (!topLevelGroups.some((group) => group.id === mobileCategoryId)) setMobileCategoryId(topLevelGroups[0].id);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => { if (event.key === "Escape") setDeleteConfirmation(null); };
  ...
  function openCategory(groupId) {
    if (isMobileStudio()) return chooseMobileCategory(groupId);
  ...
  function updateLocalized(target, id, field, language, value) {
    setMenu((current) => ({ ...current, [target]: current[target].map((entry) => entry.id === id ? { ...entry, [field]: { ...(entry[field] || {}), [language]: value } } : entry) }));
  }
  function updateEntry(target, id, patch) {
    setMenu((current) => ({ ...current, [target]: current[target].map((entry) => entry.id === id ? { ...entry, ...patch } : entry) }));
  }
  ...
  function dropCategory(event, targetId) {
    ...
    if (dragging?.type === "item") {
      setMenu((current) => moveItemToGroup(current, dragging.id, targetId));
      setDragging(null);
      return;
    }
  ...
  function dropItem(event, targetItem) {
    event.preventDefault();
    if (dragging?.type !== "item" || dragging.id === targetItem.id) return;
    if (dragging.groupId !== targetItem.group_id) {
      setMenu((current) => moveItemToGroup(current, dragging.id, targetItem.group_id));
      setDragging(null);
      return;
    }
  ...
  function addCategory(parentId = null) {
    ...
    const group = { id, name: makeLocalizedText("", "", ""), note: makeLocalizedText("", "", ""), parent_id: parentId, sort_order: nextSortOrder(menu.groups.filter((entry) => (entry.parent_id || null) === parentId)), visible: true };
  ...
  function addItem(groupId) {
    ...
    const item = { id, group_id: groupId, name: makeLocalizedText("", "", ""), description: makeLocalizedText("", "", ""), price: "", price_options: [], image_url: "", image_path: "", visible: true, sort_order: nextSortOrder(groupItems) };
  ...
  function deleteCategory(groupId) {
    ...
    const nextId = group.parent_id ? rootGroupId(nextMenu.groups, group.parent_id) : ordered(nextMenu.groups.filter((entry) => !entry.parent_id))[0]?.id || "";
  ...
  function requestDeleteCategory(groupId) {
    const group = menu.groups.find((entry) => entry.id === groupId);
    if (!group || (!group.parent_id && topLevelGroups.length <= 1)) return;
    setDeleteConfirmation({ kind: group.parent_id ? "subcategory" : "category", id: groupId, name: textValue(group.name, contentLanguage) });
  }
  function requestDeleteItem(itemId) {
    const item = menu.items.find((entry) => entry.id === itemId);
    if (item) setDeleteConfirmation({ kind: "item", id: itemId, name: textValue(item.name, contentLanguage) });
  }
  function confirmDelete() {
    if (!deleteConfirmation) return;
    const pending = deleteConfirmation;
    setDeleteConfirmation(null);
    if (pending.kind === "item") deleteItem(pending.id); else deleteCategory(pending.id);
  }
  ...
  function renderLanguageCard({ language, kind, entry, compact = false }) {
    ...
    const key = `${kind}-${entry?.id || "restaurant"}-${language}`;
    if (kind === "restaurant") return (
      <section className={`menu-content-v2-language-card ${compact ? "is-primary" : ""}`} key={key} dir={meta.dir}>
        <header><strong>{meta.label}</strong><span>{meta.code}</span></header>
        <div className="menu-content-v2-field"><label>{t.subtitle}</label><input dir={meta.dir} value={localizedFieldValue(menu.restaurant_subtitle, language)} placeholder={meta.subtitlePlaceholder} onChange={(event) => updateRestaurant("restaurant_subtitle", { ...(menu.restaurant_subtitle || {}), [language]: event.target.value })} /></div>
      </section>
    );
    if (kind === "category") return (
      <section className={`menu-content-v2-language-card ${compact ? "is-primary" : ""}`} key={key} dir={meta.dir}>
        <header><strong>{meta.label}</strong><span>{meta.code}</span></header>
        <div className="menu-content-v2-field"><label>{entry.parent_id ? t.subcategoryName : t.categoryName}</label><input dir={meta.dir} value={localizedFieldValue(entry.name, language)} placeholder={meta.categoryPlaceholder} onChange={(event) => updateLocalized("groups", entry.id, "name", language, event.target.value)} /></div>
        <div className="menu-content-v2-field menu-content-v2-group-note-field"><label>{GROUP_NOTE_COPY[uiLanguage] || GROUP_NOTE_COPY.en}</label><textarea dir={meta.dir} value={localizedFieldValue(entry.note, language)} placeholder={meta.notePlaceholder} onChange={(event) => updateLocalized("groups", entry.id, "note", language, event.target.value)} /></div>
      </section>
    );
    return (
      <section className={`menu-content-v2-language-card ${compact ? "is-primary" : ""}`} key={key} dir={meta.dir}>
        <header><strong>{meta.label}</strong><span>{meta.code}</span></header>
        <div className="menu-content-v2-field"><label>{t.itemName}</label><input dir={meta.dir} value={localizedFieldValue(entry.name, language)} placeholder={meta.itemNamePlaceholder} onChange={(event) => updateLocalized("items", entry.id, "name", language, event.target.value)} /></div>
        <div className="menu-content-v2-field"><label>{t.description}</label><textarea dir={meta.dir} value={localizedFieldValue(entry.description, language)} placeholder={meta.descriptionPlaceholder} onChange={(event) => updateLocalized("items", entry.id, "description", language, event.target.value)} /></div>
      </section>
    );
  }
  ...
  function renderTranslationSection(kind, entry, fields) {
    ...
    return (
      <section className="menu-content-v2-friendly-section menu-content-v2-translations">
        <button type="button" className="menu-content-v2-friendly-section-title" onClick={() => setTranslationsOpen((value) => !value)}>
          <span><Languages size={15} />{friendly.translations}</span>
          <span className="menu-content-v2-friendly-section-side"><small>{translationsOpen ? friendly.hideTranslations : friendly.editTranslations}</small><ChevronDown size={15} className={translationsOpen ? "open" : ""} /></span>
        </button>
        <div className="menu-content-v2-friendly-language-status">
          {otherLanguages.map((language) => {
            const present = kind === "restaurant" ? localizedFieldValue(menu.restaurant_subtitle, language).trim() : hasLocalizedContent(entry, language, fields);
            return <span key={language} className={`menu-content-v2-friendly-language-chip ${present ? "" : "missing"}`}>{MENU_LANGUAGE_META[language].code} {present ? "✓" : "·"}</span>;
          })}
        </div>
        {translationsOpen ? <div className="menu-content-v2-language-fields menu-content-v2-friendly-translation-cards">{otherLanguages.map((language) => renderLanguageCard({ language, kind, entry }))}</div> : null}
      </section>
    );
  }
  ...
  return (
    <main className="menu-content-v2" dir={rtl ? "rtl" : "ltr"} lang={uiLanguage}>
      ...
      <nav className="menu-content-v2-mobile-mode-nav" aria-label={t.content}>
        <button type="button" className={mobilePane === "structure" ? "active" : ""} onClick={() => setMobilePane("structure")}><List size={16} /><span>{t.mobileMenu}</span></button>
        <button type="button" className={mobilePane === "edit" ? "active" : ""} onClick={() => setMobilePane("edit")}><Pencil size={16} /><span>{t.mobileEdit}</span></button>
        <button type="button" className={mobilePane === "preview" ? "active" : ""} onClick={() => setMobilePane("preview")}><Eye size={16} /><span>{t.mobilePreview}</span></button>
      </nav>
      ...
      <div className={`menu-content-v2-workspace mobile-pane-${mobilePane}`}>
        <aside className="menu-content-v2-tree">
          <div className="menu-content-v2-panel-head">
            <div><span>{friendly.yourMenu}</span><strong>{friendly.editMenuHint}</strong></div>
          ...
          <label className="menu-content-v2-friendly-search"><Search size={14} /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={friendly.search} /></label>
          <p className="menu-content-v2-friendly-drag-hint">{friendly.dragHint}</p>
          ...
          <button type="button" className={`menu-content-v2-restaurant-row ${selection.type === "restaurant" ? "active" : ""}`} onClick={() => selectForEdit({ type: "restaurant", id: "restaurant" })}>
            <span className="menu-content-v2-tree-icon">B</span><span><strong>{menu.restaurant_name}</strong><small>{t.restaurantDetails}</small></span><ChevronRight size={14} />
          </button>
          ...
          <div className="menu-content-v2-categories">
            {allGroups.map((group) => {
              if (searchMatches && !searchMatches.groupIds.has(group.id) && !searchMatches.groupIds.has(rootGroupId(menu.groups, group.id))) return null;
              const allItems = ordered(menu.items.filter((item) => item.group_id === group.id));
              const items = searchMatches ? allItems.filter((item) => searchMatches.itemIds.has(item.id) || searchableText(group.name).includes(searchNeedle)) : allItems;
              const selectedInGroup = selection.type === "item" && allItems.some((item) => item.id === selection.id);
              const isOpen = Boolean(searchNeedle) || (selection.type === "category" && selection.id === group.id) || selectedInGroup;
              const groupDragging = dragging?.type === "category" && dragging.id === group.id;
              return (
                <section key={group.id} className={`menu-content-v2-category-block ${groupDragging ? "dragging" : ""} ${isOpen ? "is-open" : ""}`}>
                  <button type="button" draggable className={`menu-content-v2-category-row ${selection.type === "category" && selection.id === group.id ? "active" : ""}`}
                    onClick={() => openCategory(group.id)} onDragStart={(event) => beginDrag(event, { type: "category", id: group.id })}
                    onDragOver={(event) => dragging && event.preventDefault()} onDrop={(event) => dropCategory(event, group.id)} onDragEnd={() => setDragging(null)}>
                    <GripVertical size={13} /><span><strong dir={contentDir}>{textValue(group.name, contentLanguage) || (group.parent_id ? t.subcategoryName : t.categoryName)}</strong><small>{t.items(allItems.length)}</small></span><ChevronRight size={13} />
                  </button>

                  {isOpen ? <div className="menu-content-v2-items">
                    {items.map((item) => {
                      const summary = priceSummary(item, currencySymbol, contentLanguage);
                      const itemDragging = dragging?.type === "item" && dragging.id === item.id;
                      return <button type="button" draggable key={item.id} className={`${selection.type === "item" && selection.id === item.id ? "active" : ""} ${itemDragging ? "dragging" : ""}`.trim()}
                        onClick={() => selectForEdit({ type: "item", id: item.id })} onDragStart={(event) => beginDrag(event, { type: "item", id: item.id, groupId: group.id })}
                        onDragOver={(event) => dragging?.type === "item" && event.preventDefault()} onDrop={(event) => dropItem(event, item)} onDragEnd={() => setDragging(null)}>
                        <GripVertical size={11} /><span><strong dir={contentDir}>{textValue(item.name, contentLanguage) || t.itemName}</strong><small>{summary || t.noPrice}</small></span>
                      </button>;
                    })}
                    {!searchNeedle ? <button type="button" className="menu-content-v2-add-item" onClick={() => addItem(group.id)}><Plus size={12} /> {friendly.addTo(textValue(group.name, contentLanguage))}</button> : null}
                  </div> : null}
                </section>
              );
            })}
            {searchMatches && searchMatches.groupIds.size === 0 ? <div className="menu-content-v2-friendly-empty">{friendly.noResults}</div> : null}
          </div>
        ...
        <section className="menu-content-v2-preview">
          <div className="menu-content-v2-canvas-toolbar"><div><span className="live" /><strong>{t.livePreview}</strong></div></div>
          <div className="menu-content-v2-canvas"><MenuStudioMobilePreview menu={menu} design={design} language={contentLanguage} minScale={0.34} maxScale={0.9} /></div>
        </section>

        <aside className="menu-content-v2-inspector">
          <div className="menu-content-v2-mobile-editor-bar"><button type="button" onClick={() => setMobilePane("structure")}><BackIcon size={17} /> {t.backToMenu}</button><button type="button" onClick={() => setMobilePane("preview")}><Eye size={16} /> {t.mobilePreview}</button></div>

          {selection.type === "restaurant" ? <>
            <div className="menu-content-v2-inspector-head"><span>{t.restaurantEyebrow}</span><h2>{t.menuDetails}</h2></div>
            <div className="menu-content-v2-primary-editor">
              ...
              {renderLanguageCard({ language: primaryLanguage, kind: "restaurant", entry: menu, compact: true })}
            </div>
            {renderTranslationSection("restaurant", menu, ["restaurant_subtitle"])}
          </> : null}

          {selectedCategory ? <>
            <div className="menu-content-v2-inspector-head"><span>{selectedCategory.parent_id ? t.subcategoryEyebrow : t.categoryEyebrow}</span><h2 dir={contentDir}>{textValue(selectedCategory.name, contentLanguage) || (selectedCategory.parent_id ? t.subcategoryName : t.categoryName)}</h2></div>
            <div className="menu-content-v2-primary-editor">{renderLanguageCard({ language: primaryLanguage, kind: "category", entry: selectedCategory, compact: true })}</div>
            {renderTranslationSection("category", selectedCategory, ["name", "note"])}
            <button type="button" className="menu-content-v2-inspector-add menu-content-v2-primary-add" onClick={() => addItem(selectedCategory.id)}><Plus size={14} /> {friendly.addTo(textValue(selectedCategory.name, contentLanguage))}</button>
            <section className="menu-content-v2-friendly-section">
              <button type="button" className="menu-content-v2-friendly-section-title" onClick={() => setAdvancedOpen((value) => !value)}><span><SlidersHorizontal size={15} />{friendly.moreOptions}</span><span className="menu-content-v2-friendly-section-side"><small>{advancedOpen ? friendly.hideOptions : ""}</small><ChevronDown size={15} className={advancedOpen ? "open" : ""} /></span></button>
              {advancedOpen ? <div className="menu-content-v2-friendly-advanced-body">
          ...
          {selectedItem ? <>
            <div className="menu-content-v2-inspector-head"><span>{t.itemEyebrow}</span><h2 dir={contentDir}>{textValue(selectedItem.name, contentLanguage) || t.itemName}</h2></div>
            <div className="menu-content-v2-primary-editor">{renderLanguageCard({ language: primaryLanguage, kind: "item", entry: selectedItem, compact: true })}</div>
            <MenuContentPriceEditor item={selectedItem} currencySymbol={currencySymbol} contentLanguage={contentLanguage} contentDir={contentDir} t={t} onChange={(patch) => updateEntry("items", selectedItem.id, patch)} />
            <MenuContentImageEditor item={selectedItem} projectId={menu.source_project_id || profile?.importedProjectId || "draft"} t={t} onChange={(patch) => updateEntry("items", selectedItem.id, patch)} />
            {renderTranslationSection("item", selectedItem, ["name", "description"])}
            <section className="menu-content-v2-friendly-section">
              <button type="button" className="menu-content-v2-friendly-section-title" onClick={() => setAdvancedOpen((value) => !value)}><span><SlidersHorizontal size={15} />{friendly.moreOptions}</span><span className="menu-content-v2-friendly-section-side"><small>{advancedOpen ? friendly.hideOptions : ""}</small><ChevronDown size={15} className={advancedOpen ? "open" : ""} /></span></button>
              {advancedOpen ? <div className="menu-content-v2-friendly-advanced-body">
                <div className="menu-content-v2-field"><label htmlFor="studio-item-group">{t.itemCategory}</label><select id="studio-item-group" value={selectedItem.group_id} onChange={(event) => { const groupId = event.target.value; setMenu((current) => moveItemToGroup(current, selectedItem.id, groupId)); setMobileCategoryId(rootGroupId(menu.groups, groupId)); }}>{allGroups.map((group) => { const parent = menu.groups.find((entry) => entry.id === group.parent_id); const name = textValue(group.name, contentLanguage) || (parent ? t.subcategoryName : t.categoryName); return <option key={group.id} value={group.id}>{parent ? `${textValue(parent.name, contentLanguage) || t.categoryName} / ${name}` : name}</option>; })}</select></div>
      ...
      {deleteConfirmation ? <div className="menu-content-v2-confirm-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDeleteConfirmation(null); }}>
        <section className="menu-content-v2-confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="menu-content-v2-confirm-title" aria-describedby="menu-content-v2-confirm-message">
          <span className="icon" aria-hidden="true"><Trash2 size={20} /></span><h2 id="menu-content-v2-confirm-title">{deleteDialogTitle}</h2><p id="menu-content-v2-confirm-message">{deleteDialogMessage}</p>
          <div className="menu-content-v2-confirm-actions"><button type="button" className="cancel" onClick={() => setDeleteConfirmation(null)}>{deleteCopy.cancel}</button><button type="button" className="delete" onClick={confirmDelete} autoFocus>{deleteCopy.confirm}</button></div>
        </section>
      </div> : null}
    </main>
  );

...
import "../features/menu-engine/studio/MenuStudioPhonePreview.css";
...
const UI = {
  en: {
    ...
    realDraft:"MENU CONTENT", designDraft:"DESIGN", makeItYours:"Make it yours", designHint:"Choose a design direction, then refine every detail while the live menu updates instantly.",
    classic:"Classic", visual:"Visual", classicHint:"Typography-led menus", visualHint:"Image-led menus", badges:"Badges", showBadgeSymbols:"Badge symbols",
  ...
  he: {
    ...
    makeItYours:"עצבו את התפריט", designHint:"בחרו כיוון עיצובי ולאחר מכן התאימו כל פרט בזמן שהתפריט החי מתעדכן מיד.", classic:"קלאסי", visual:"ויזואלי",
    classicHint:"תפריטים מבוססי טיפוגרפיה", visualHint:"תפריטים מבוססי תמונה", badges:"תגיות", showBadgeSymbols:"סמלים בתגיות", withSymbols:"סמל + טקסט",
  ...
  ar: {
    ...
    designHint:"اختر اتجاه التصميم ثم خصص كل التفاصيل بينما تتحدث القائمة المباشرة فورًا.", classic:"كلاسيكي", visual:"مرئي", classicHint:"قوائم تعتمد على الخطوط",
    visualHint:"قوائم تعتمد على الصور", badges:"الشارات", showBadgeSymbols:"رموز الشارات", withSymbols:"رمز + نص", textOnly:"نص فقط",
...
export default function MenuStudioV3Draft() {
  ...
  function addItem() { if (!selectedGroup) return; const id = draftId("item"); const directItems = menu.items.filter(item => item.group_id === selectedGroup.id); const item = { id, group_id:selectedGroup.id, name:emptyLanguageText(contentLanguage,""), description:emptyLanguageText(contentLanguage,""), price:"", price_options:[], image_url:"", image_path:"", visible:true, sort_order:directItems.length + 1, metadata:{ allergens:[], dietary:[], merchandising:[], spice:"none", aiSuggestions:[], reviewedByOwner:false } }; patchMenu(current => ({ ...current, items:[...current.items,item] })); setEditingItemId(id); setDraftItem({ ...item, __contentLanguage:contentLanguage }); }

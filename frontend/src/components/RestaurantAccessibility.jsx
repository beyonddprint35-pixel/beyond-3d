...
function CloseIcon(){
  return <svg className="restaurant-a11y-close-icon" viewBox="0 0 24 24" aria-hidden="true" width="20" height="20" fill="none" stroke="#111" strokeWidth="2.4" strokeLinecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg>;
}
...
export default function RestaurantAccessibility({ restaurantName="Restaurant", language="he" }) {
  ...
  const [previewPortalTarget,setPreviewPortalTarget] = useState(null);
  ...
  useEffect(()=>{
    ...
    const previewScreen=anchor?.closest?.(".studio-v3-preview-device-screen");
  ...
  return <>
    <span ref={anchorRef} className="restaurant-a11y-preview-anchor" aria-hidden="true" />
    {previewPortalTarget ? createPortal(controls,previewPortalTarget) : controls}
  </>;
}

...
const labels = {
  en: {
    presets:"Style presets", restore:"Restore original design", restoreHint:"Return to the design this menu started with",
    brand:"Brand", colors:"Colors", typography:"Typography", layout:"Layout", uploadLogo:"Upload logo", replaceLogo:"Replace logo",
    logoSize:"Logo size", logoShape:"Logo shape", removeLogo:"Remove logo", heroBackground:"Menu header background",
    watermark:"Logo watermark", image:"Background image", none:"No image", watermarkHint:"Use the restaurant logo inside the classic pill-shaped background.",
    imageHint:"Upload a separate image for the menu header background.", uploadBackground:"Upload background", replaceBackground:"Replace background", removeBackground:"Remove background",
    headingFont:"Heading font", bodyFont:"Body font", headingWeight:"Heading weight", itemWeight:"Item weight", text:"Main text", muted:"Secondary text",
    accent:"Accent", background:"Background", cards:"Cards", category:"Categories", categoryText:"Category text", density:"Density",
    navigation:"Category navigation", price:"Price position", imagePosition:"Image position", imageRatio:"Image ratio",
  },
  he: {
    presets:"סגנונות מוכנים", restore:"שחזור העיצוב המקורי", restoreHint:"חזרה לעיצוב שממנו התפריט התחיל",
    brand:"מותג", colors:"צבעים", typography:"טיפוגרפיה", layout:"פריסה", uploadLogo:"העלאת לוגו", replaceLogo:"החלפת לוגו",
    logoSize:"גודל לוגו", logoShape:"צורת לוגו", removeLogo:"הסר לוגו", heroBackground:"רקע כותרת התפריט",
    watermark:"לוגו כסימן מים", image:"תמונת רקע", none:"ללא תמונה", watermarkHint:"הצגת לוגו המסעדה בתוך הרקע המלבני המעוגל הקלאסי.",
    imageHint:"העלאת תמונה נפרדת כרקע לכותרת התפריט.", uploadBackground:"העלאת רקע", replaceBackground:"החלפת רקע", removeBackground:"הסר רקע",
    headingFont:"פונט כותרות", bodyFont:"פונט טקסט", headingWeight:"עובי כותרות", itemWeight:"עובי שמות פריטים", text:"טקסט ראשי", muted:"טקסט משני",
    accent:"צבע מוביל", background:"רקע", cards:"כרטיסים", category:"קטגוריות", categoryText:"טקסט קטגוריות", density:"צפיפות",
    navigation:"ניווט קטגוריות", price:"מיקום מחיר", imagePosition:"מיקום תמונה", imageRatio:"יחס תמונה",
  },
  ar: {
    presets:"أنماط جاهزة", restore:"استعادة التصميم الأصلي", restoreHint:"العودة إلى التصميم الذي بدأت به القائمة",
    brand:"العلامة", colors:"الألوان", typography:"الخطوط", layout:"التخطيط", uploadLogo:"رفع الشعار", replaceLogo:"استبدال الشعار",
    logoSize:"حجم الشعار", logoShape:"شكل الشعار", removeLogo:"إزالة الشعار", heroBackground:"خلفية عنوان القائمة",
    watermark:"الشعار كعلامة مائية", image:"صورة خلفية", none:"بدون صورة", watermarkHint:"استخدم شعار المطعم داخل الخلفية المستطيلة المستديرة الكلاسيكية.",
    imageHint:"ارفع صورة منفصلة لخلفية عنوان القائمة.", uploadBackground:"رفع الخلفية", replaceBackground:"استبدال الخلفية", removeBackground:"إزالة الخلفية",
    headingFont:"خط العناوين", bodyFont:"خط النص", headingWeight:"سماكة العناوين", itemWeight:"سماكة أسماء العناصر", text:"النص الرئيسي", muted:"النص الثانوي",
    accent:"اللون الرئيسي", background:"الخلفية", cards:"البطاقات", category:"الفئات", categoryText:"نص الفئات", density:"الكثافة",
    navigation:"تنقل الفئات", price:"موضع السعر", imagePosition:"موضع الصورة", imageRatio:"نسبة الصورة",
  },
};
...
export default function MenuDesignControls({design,baselineDesign,menu,language="en",panel,setPanel,patchDesign}) {
  ...
  return <div className="studio-v3-design-pro-controls">
    <div className="studio-v3-design-section studio-v3-restore-section">
      <strong>{l.presets}</strong>
      <button type="button" className={`studio-v3-original-design ${isOriginal?"active":""}`} disabled={!original} onClick={()=>original&&patchDesign(()=>original)}>
        <span><strong>{l.restore}</strong><small>{l.restoreHint}</small></span><span aria-hidden="true">↺</span>
      </button>
      <div className="studio-v3-design-preset-grid">{Object.keys(MENU_DESIGN_PRESETS).map(key=><Choice key={key} onClick={()=>patchDesign(current=>applyMenuDesignPreset(current,key))}>{titleCase(key)}</Choice>)}</div>
    </div>
    ...
    {panel==="brand"&&<div className="studio-v3-design-section">
      ...
      <div className="studio-v3-hero-background-control">
        <div className="studio-v3-design-control-label">{l.heroBackground}</div>
        <div className="studio-v3-hero-mode-grid">
          <Choice active={heroMode==="watermark"} onClick={()=>patchBrand("heroMediaMode","watermark")}>{l.watermark}</Choice>
          <Choice active={heroMode==="image"} onClick={()=>patchBrand("heroMediaMode","image")}>{l.image}</Choice>
          <Choice active={heroMode==="none"} onClick={()=>patchBrand("heroMediaMode","none")}>{l.none}</Choice>
        </div>
        {heroMode==="watermark"?<p className="studio-v3-control-help">{l.watermarkHint}</p>:null}
        {heroMode==="image"?<div className="studio-v3-background-upload"><p className="studio-v3-control-help">{l.imageHint}</p>{heroImage?<img src={heroImage} alt=""/>:null}<label className="studio-v3-logo-upload-button"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>uploadImage(e.target.files?.[0],"heroImageUrl")}/><span>{heroImage?l.replaceBackground:l.uploadBackground}</span></label>{heroImage?<button className="studio-v3-design-remove-logo" type="button" onClick={()=>patchBrand("heroImageUrl","")}>{l.removeBackground}</button>:null}</div>:null}
      </div>
    </div>}

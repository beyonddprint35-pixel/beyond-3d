...
export default function MenuDesignControls({design,baselineDesign,menu,language="en",panel,setPanel,patchDesign}){
  ...
  const panelMeta={
    ...
    badges:{icon:"◆",title:l.badges,hint:c.badgesHint},
  ...
  return <div className="studio-v3-design-pro-controls">
    ...
    {workspaceMode==="customize"?<div className="studio-v3-customize-workspace">
      <section className="studio-v3-inspector-shell">
        ...
        <div className="studio-v3-design-tabs">{[["brand","✦",l.brand],["colors","◉",l.colors],["type","Aa",l.typography],["layout","⌗",l.layout],["badges","◆",l.badges]].map(([key,icon,label])=><button type="button" key={key} className={panel===key?"active":""} onClick={()=>setPanel(key)}><span className="studio-v3-design-tab-icon">{icon}</span><span className="studio-v3-design-tab-label">{label}</span></button>)}</div>
        ...
        {panel==="badges"?<div className="studio-v3-design-section studio-v3-inspector-panel studio-v3-badges-inspector">
          <section className="studio-v3-inspector-block">
            <div className="studio-v3-inspector-block-title"><span>{c.badgeDisplay}</span><small>{l.badgeSymbols}</small></div>
            <div className="studio-v3-badge-display-grid">
              <button type="button" className={`studio-v3-badge-choice ${design.badges.showSymbols?"active":""}`} onClick={()=>patchBadges("showSymbols",true)}><span className="studio-v3-badge-choice-preview symbol"><i>◆</i><b>Popular</b></span><strong>{l.iconText}</strong></button>
              <button type="button" className={`studio-v3-badge-choice ${!design.badges.showSymbols?"active":""}`} onClick={()=>patchBadges("showSymbols",false)}><span className="studio-v3-badge-choice-preview text"><b>Popular</b></span><strong>{l.textOnly}</strong></button>
            </div>
          </section>
          <section className="studio-v3-inspector-block">
            <div className="studio-v3-inspector-block-title"><span>{c.badgeAppearance}</span><small>{l.badgeStyle}</small></div>
            <div className="studio-v3-badge-style-grid">{[["auto",l.autoStyle],["minimal",l.minimalStyle],["filled",l.filledStyle],["playful",l.playfulStyle]].map(([value,label])=><button type="button" key={value} className={`studio-v3-badge-style-choice style-${value} ${design.badges.iconStyle===value?"active":""}`} disabled={!design.badges.showSymbols} onClick={()=>patchBadges("iconStyle",value)}><span className="studio-v3-badge-style-preview"><i>◆</i><b>Chef</b></span><strong>{label}</strong></button>)}</div>
            <p className="studio-v3-inspector-help">{c.badgeAutoHint}</p>
          </section>
        </div>:null}
      </section>
    </div>:null}

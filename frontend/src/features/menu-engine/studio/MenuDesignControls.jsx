...
export default function MenuDesignControls({design,baselineDesign,menu,language="en",panel,setPanel,patchDesign}){
  ...
  const filteredDesigns=useMemo(()=>{
    ...
    return PREMIUM_MENU_DESIGNS.filter(entry=>[entry.name,entry.category,entry.description,...entry.tags].join(" ").toLowerCase().includes(needle));
  },[query]);
  ...
  return <div className="studio-v3-design-pro-controls">
    <section className="studio-v3-design-library">
      ...
      <div className="studio-v3-premium-library-grid">{filteredDesigns.map(entry=><button type="button" className="studio-v3-premium-design-card" key={entry.id} onClick={()=>patchDesign(current=>applyPremiumMenuDesign(current,entry.id))}><span className="studio-v3-premium-design-preview" style={{background:entry.swatches[0]}}><i style={{background:entry.swatches[1]}}/><i style={{background:entry.swatches[2]}}/></span><span className="studio-v3-premium-design-copy"><span><strong>{entry.name}</strong><small>{entry.category}</small></span><em>{entry.description}</em></span></button>)}</div>

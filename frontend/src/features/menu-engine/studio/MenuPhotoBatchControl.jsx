...
export default function MenuPhotoBatchControl({ menu, design, siteId, slug, language="en", onApplyItemPatches }) {
  ...
  const aiFinished = useMemo(() => photoItems.filter(item => item.image_finish_profile === PRO_FINISH_PROFILE && String(item.image_finish_source || "").includes("ai-vision")).length, [photoItems]);
  const localFinished = useMemo(() => photoItems.filter(item => item.image_finish_profile === PRO_FINISH_PROFILE && item.image_finish_source && !String(item.image_finish_source).includes("ai-vision")).length, [photoItems]);
  ...
  async function matchAll() {
    ...
    try {
      ...
      if (failed.length) {
        ...
        setMessage(`${copy.done}${successful.length ? ` · ${copy.cacheDone}` : ""}`);

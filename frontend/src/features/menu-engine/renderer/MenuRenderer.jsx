...
function HeritageClassicRenderer({menu,design,language,setLanguage,languages,topGroups,activeGroup,onSelectGroup,blocks,totalItems,currencySymbol,logoUrl}){
  ...
  const showHeroPhoto=heroMode==="image"&&Boolean(heroImageUrl);
  const showHeaderLogo=heroMode!=="none";
  const appRows=showHeroPhoto?undefined:{gridTemplateRows:"auto auto auto minmax(0,1fr) auto"};
  return <div className={`ep-page customers-template-menu bme-heritage-exact bme-density-${design.layout.density} bme-nav-${design.layout.navigationStyle} bme-logo-${design.brand.logoShape} bme-badge-style-${badgeStyle}`} lang={language} dir={rtl?"rtl":"ltr"} style={heritageVariables(design)}>
    ...
    <div id="restaurant-main-content" className={`ep-app ep-lang-${language} ${showHeroPhoto?"":"ep-no-hero"}`} style={appRows} tabIndex={-1} lang={language} dir={rtl?"rtl":"ltr"}>
      <header className="ep-header"><div className="ep-brand" dir="ltr">{showHeaderLogo?(logoUrl?<img className="ep-logo" src={logoUrl} alt=""/>:<div className="ep-logo ep-logo-fallback">B</div>):null}<div className="ep-brand-copy" dir="ltr"><div className="ep-brand-title">{menu.restaurant_name}</div>{menu.restaurant_subtitle?<div className="ep-brand-sub">{chooseText(language,menu.restaurant_subtitle)}</div>:null}</div></div>{languages.length>1?<div className="ep-lang-pill" aria-label="Menu language">{languages.map(code=><button type="button" key={code} lang={code} dir={isRtl(code)?"rtl":"ltr"} aria-pressed={language===code} className={language===code?"active":""} onClick={()=>setLanguage(code)}>{LANGUAGE_LABELS[code]||code.toUpperCase()}</button>)}</div>:null}</header>
      {showHeroPhoto?<section className="ep-hero">{menu.hero_eyebrow?<div className="ep-hero-kicker">{chooseText(language,menu.hero_eyebrow)}</div>:null}{menu.hero_title?<h1 className="ep-hero-title">{chooseText(language,menu.hero_title)}</h1>:null}<img className="ep-hero-background-image" src={heroImageUrl} alt="" aria-hidden="true"/></section>:null}

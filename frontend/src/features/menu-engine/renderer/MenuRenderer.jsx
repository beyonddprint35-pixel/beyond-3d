...
import "./menuGroupNotes.css";
import "./menuTextRoleColors.css";
...
const TEXT_ROLE_COLORS = Object.freeze({
  brandTitleColor:"brand-title",
  brandSubtitleColor:"brand-subtitle",
  heroKickerColor:"hero-kicker",
  heroTitleColor:"hero-title",
  categoryNavColor:"category-nav",
  sectionTitleColor:"section-title",
  subcategoryTitleColor:"subcategory-title",
...
function clampHeroZoom(value){const number=Number(value);return Number.isFinite(number)?Math.min(3,Math.max(1,number)):1;}
function textRoleVariables(prefix,theme={}){const variables={};Object.entries(TEXT_ROLE_COLORS).forEach(([key,slug])=>{const value=String(theme?.[key]||"").trim();if(value)variables[`--${prefix}-${slug}-color`]=value;});return variables;}
function textRoleTypeVariables(prefix,typography={}){const variables={};const roles=typography?.roles&&typeof typography.roles==="object"&&!Array.isArray(typography.roles)?typography.roles:{};const allowed=new Set(Object.values(TEXT_ROLE_COLORS));Object.entries(roles).forEach(([slug,style])=>{if(!allowed.has(slug)||!style||typeof style!=="object"||Array.isArray(style))return;if(Object.prototype.hasOwnProperty.call(style,"size")){const size=Math.min(72,Math.max(8,Number(style.size)||16));variables[`--${prefix}-role-${slug}-size`]=`${size}px`;}if(typeof style.bold==="boolean")variables[`--${prefix}-role-${slug}-weight`]=style.bold?700:400;if(typeof style.italic==="boolean")variables[`--${prefix}-role-${slug}-style`]=style.italic?"italic":"normal";if(typeof style.underline==="boolean")variables[`--${prefix}-role-${slug}-decoration`]=style.underline?"underline":"none";});return variables;}
function textRoleClasses(design={}){const theme=design?.theme||{};const typography=design?.typography||{};const colorClasses=Object.entries(TEXT_ROLE_COLORS).filter(([key])=>String(theme?.[key]||"").trim()).map(([,slug])=>`bme-custom-color-${slug}`);const allowed=new Set(Object.values(TEXT_ROLE_COLORS));const roles=typography?.roles&&typeof typography.roles==="object"&&!Array.isArray(typography.roles)?typography.roles:{};const typeClasses=Object.entries(roles).filter(([slug,style])=>allowed.has(slug)&&style&&typeof style==="object"&&!Array.isArray(style)&&["size","bold","italic","underline"].some(key=>Object.prototype.hasOwnProperty.call(style,key))).map(([slug])=>`bme-custom-type-${slug}`);return [...colorClasses,...typeClasses].join(" ");}
function designVariables(design){return{"--bme-bg":design.theme.background,"--bme-surface":design.theme.surface,"--bme-card":design.theme.card,"--bme-text":design.theme.text,"--bme-muted":design.theme.muted,"--bme-accent":design.theme.accent,"--bme-accent-secondary":design.theme.accentSecondary,"--bme-line":design.theme.line,"--bme-category-bg":design.theme.categoryBackground,"--bme-category-text":design.theme.categoryText,"--bme-item-name-color":design.theme.itemNameColor||design.theme.text,...textRoleVariables("bme",design.theme),...textRoleTypeVariables("bme",design.typography),"--bme-heading-font":fontStack(design.typography.headingFont,true),"--bme-body-font":fontStack(design.typography.bodyFont,false),"--bme-number-font":fontStack(design.typography.numberFont,true),"--bme-heading-weight":design.typography.headingWeight,"--bme-body-weight":design.typography.bodyWeight,"--bme-item-weight":design.typography.itemWeight,"--bme-brand-size":`${design.typography.brandSize}px`,"--bme-hero-size":`${design.typography.heroSize}px`,"--bme-section-size":`${design.typography.sectionSize}px`,"--bme-category-size":`${design.typography.categorySize}px`,"--bme-item-size":`${design.typography.itemNameSize}px`,"--bme-description-size":`${design.typography.descriptionSize}px`,"--bme-price-size":`${design.typography.priceSize}px`,"--bme-logo-size":`${design.brand.logoSize}px`,"--bme-radius":`${design.layout.cardRadius}px`,"--bme-section-gap":`${design.layout.sectionGap}px`,"--bme-item-gap":`${design.layout.itemGap}px`,"--bme-card-padding":`${design.layout.cardPadding}px`,"--bme-hero-focus-x":`${clampFocus(design.brand.heroImageFocusX)}%`,"--bme-hero-focus-y":`${clampFocus(design.brand.heroImageFocusY)}%`,"--bme-hero-image-zoom":clampHeroZoom(design.brand.heroImageZoom)};}
function heritageVariables(design){return{"--ep-bg":design.theme.background,"--ep-paper":design.theme.surface,"--ep-card":design.theme.card,"--ep-ink":design.theme.text,"--ep-muted":design.theme.muted,"--ep-line":design.theme.line,"--ep-accent":design.theme.accent,"--ep-accent-secondary":design.theme.accentSecondary,"--ep-category-bg":design.theme.categoryBackground,"--ep-category-text":design.theme.categoryText,"--ep-item-name-color":design.theme.itemNameColor||design.theme.text,...textRoleVariables("ep",design.theme),...textRoleTypeVariables("ep",design.typography),"--ep-heading-font":fontStack(design.typography.headingFont,true),"--ep-body-font":fontStack(design.typography.bodyFont,false),"--ep-number-font":fontStack(design.typography.numberFont,true),"--ep-heading-weight":design.typography.headingWeight,"--ep-body-weight":design.typography.bodyWeight,"--ep-item-weight":design.typography.itemWeight,"--ep-brand-size":`${design.typography.brandSize}px`,"--ep-hero-size":`${design.typography.heroSize}px`,"--ep-section-size":`${design.typography.sectionSize}px`,"--ep-category-size":`${design.typography.categorySize}px`,"--ep-item-size":`${design.typography.itemNameSize}px`,"--ep-description-size":`${design.typography.descriptionSize}px`,"--ep-price-size":`${design.typography.priceSize}px`,"--ep-logo-size":`${design.brand.logoSize}px`,"--ep-radius":`${design.layout.cardRadius}px`,"--ep-section-gap":`${design.layout.sectionGap}px`,"--ep-item-gap":`${design.layout.itemGap}px`,"--ep-card-padding":`${design.layout.cardPadding}px`,"--ep-hero-focus-x":`${clampFocus(design.brand.heroImageFocusX)}%`,"--ep-hero-focus-y":`${clampFocus(design.brand.heroImageFocusY)}%`,"--ep-hero-image-zoom":clampHeroZoom(design.brand.heroImageZoom)};}
...
function HeritageClassicRenderer({menu,design,language,setLanguage,languages,topGroups,activeGroup,onSelectGroup,blocks,totalItems,currencySymbol,logoUrl,onItemImpression,onItemOpen}){
  ...
  const activeNote=activeGroup?chooseText(language,activeGroup.note):"";
  const roleClasses=textRoleClasses(design);
  return <div className={`ep-page customers-template-menu bme-heritage-exact bme-density-${design.layout.density} bme-nav-${design.layout.navigationStyle} bme-logo-${design.brand.logoShape} bme-badge-style-${badgeStyle} ${roleClasses}`} lang={language} dir={rtl?"rtl":"ltr"} style={heritageVariables(design)}>
    ...
    <div id="restaurant-main-content" className={`ep-app ep-lang-${language} ${showHero?"":"ep-no-hero"}`} style={appRows} tabIndex={-1} lang={language} dir={rtl?"rtl":"ltr"}>
      ...
      <section className="ep-section-head"><div><h2>{activeGroup?chooseText(language,activeGroup.name):""}</h2>{activeNote?<p className="ep-group-note ep-group-note-primary">{activeNote}</p>:null}</div><div className="ep-section-count">{totalItems} {footer.items}</div></section>
      <section className="ep-menu-list">{blocks.map((block,index)=>{const note=chooseText(language,block.group.note);return <div key={block.group.id}>{index>0?<div className="ep-item-category-wrap"><div className="ep-item-category">{chooseText(language,block.group.name)}</div>{note?<p className="ep-group-note ep-group-note-subcategory">{note}</p>:null}</div>:null}{block.items.map(item=><HeritageItem key={item.id} item={item} language={language} currencySymbol={currencySymbol} design={design} onImpression={onItemImpression} onOpen={onItemOpen}/>)}</div>;})}</section>
...
function MenuRenderer({menu,design:designInput,initialLanguage,onAnalyticsEvent}){
  ...
  const activeNote=activeGroup?chooseText(language,activeGroup.note):"";
  return <div className={`bme-menu bme-template-${design.template} bme-presentation-${presentation} bme-density-${design.layout.density} bme-nav-${design.layout.navigationStyle} bme-logo-${design.brand.logoShape} bme-badge-style-${badgeStyle} ${roleClasses}`} style={style} dir={rtl?"rtl":"ltr"} lang={language}>

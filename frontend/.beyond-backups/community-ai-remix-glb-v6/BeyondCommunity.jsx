...
function BeyondCommunity({
  ...
  async function remixAiModel(item) {
    ...
    try {
      ...
      let model3mfUrl = payload.model3mfUrl || null;

      if (!model3mfUrl && item.user_id === session.user.id) {
        const assets = await resolveOwnedAiGenerationAssets({
          item,
          userId: session.user.id,
        });
        ...
        model3mfUrl =
          assets?.model_3mf_signed_url ||
          assets?.model_3mf_url ||
          null;
      }

      if (!model3mfUrl) {
        model3mfUrl = `/.netlify/functions/get-community-ai-model?id=${encodeURIComponent(
          item.id
        )}&format=3mf`;
      }
      ...
      setError(remixError?.message || "Could not open this AI model in Creator.");
    } finally {
      setBusyId(null);
    }
  }

...
function BeyondCreator({
  ...
  // BEYOND_COMMUNITY_AI_REMIX_IMPORT_V6
  // AI Community Remix is an explicit NEW IMPORT. It is therefore allowed
  // to perform one camera fit after loading. Normal selection/move/drag still
  // never recenters the Studio camera.
  useEffect(() => {
    async function handleCommunityAiRemix(event) {
      const detail = event?.detail || {};
      const modelUrl = detail.modelUrl;
      ...
      try {
        ...
        if (!importedObject) {
          throw new Error(
            "The AI file downloaded, but Creator could not convert it into an editable mesh."
          );
        }

        // Explicit one-time fit for a newly imported AI object. This does NOT
        // re-enable camera-follow on normal transforms.
        window.setTimeout(() => {
          const camera =
        ...
        setOperationMessage(
          "AI model opened in Studio as an editable mesh. Save it to create your own Remix project."
        );
      } catch (error) {
        console.error(

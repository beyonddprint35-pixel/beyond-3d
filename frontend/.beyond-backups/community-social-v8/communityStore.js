...
function isCommunityCommentsSetupError(error) {
...
export async function listCommunityCommentCounts({ limit = 5000 } = {}) {
  ...
  if (error) {
    // Discussion is optional. A missing/new comments table must NEVER
    // prevent Community creations, likes or 3D previews from loading.
    if (isCommunityCommentsSetupError(error)) {
      console.warn(
        "[BEYOND COMMUNITY] Comments table is not ready yet; loading Community without comments.",
        error
      );
      return [];
    }
    ...
    console.warn(
      "[BEYOND COMMUNITY] Comment counts failed; loading Community without counts.",
      error
    );
    return [];
  }
...
export async function listCommunityComments({ itemId, limit = 120 } = {}) {
  ...
  if (error) {
    if (isCommunityCommentsSetupError(error)) {
      console.warn(
        "[BEYOND COMMUNITY] Comments table is not ready yet.",
        error
      );
      return [];
    }
...
export async function addCommunityComment({
  ...
  if (error) {
    if (isCommunityCommentsSetupError(error)) {
      throw new Error(
        "Community comments are not set up yet. Run the current supabase-projects.sql in Supabase SQL Editor."
      );
    }
    throw error;
  }
